...
class Game:
	def __init__(self):
		...
		for x in range(0,5):
                        for y in range(0,5):
                                self.canvas.create_image(x*w,y*h,image=self.bg,ancher='nw')
                self.sprites=[]
                self.running=True
...
class StickFigureSprite(Sprite):
        def __init__(self,game):
                ...
                game.canvas.bind-all('<KeyPress-Right>',self.turn_right)
                game.canvas.bind_all('<space>',self.jump)
        def turn_left(self,evt):
                if self.y==0:
                        self.x=-2
        def turn_rught(self,evt):
                if self.y==0:
                        self.x=2
        def jump(self,evt):
                if self.y==0:
        ...
        def animate(self):
                ...
                if self.x<0:
                        if.self.y !=0:
                                ...
                                self.game.canvas.itemconfig(self.image,image=self.images_left[left.current_image])
                elif self.x>0:
                        if self.y !=0:
        ...
        def move(self):
            ...
            if self.x>0 and co.x2>=self.game.canvas_width:    
                ...
                self.x=0
                left=False
            for sprite in self.ganme.sprites:
                if sprite==self:
                    continue
                ...
                if right and self.x>0 collided_right(co,sprite_co):                                          
                    self.x=0
                    right=False
                    if sprite.endgame:
                        self.game.running=False
...
class DooeSprite(Sprite):
    def __init__(self,game,ptoto_image,x,y,width,height):
        Sprite.__init__(self,game)                                                    
        self.photo_image=photo-image                                                    
        self.imgae=game.canvas.create_image(x,y,image=self.photo_image,anchor='nw')                                            
